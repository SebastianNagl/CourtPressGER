...
def generate_summ_data_mgpu(config):
    """
    ...
    if num_rows == 0:
        print("[generate_summ_data_mgpu] The input CSV is empty. Exiting.")
        return

    gpu_count = config.get("gpu_count", 1)
    if gpu_count < 1:
        print(f"[generate_summ_data_mgpu] Invalid gpu_count={gpu_count}. Using 1 GPU instead.")
        gpu_count = 1

    print(f"[generate_summ_data_mgpu] Loaded {num_rows} rows from {config['input']}.")
    print(f"[generate_summ_data_mgpu] Splitting data for {gpu_count} GPUs...")
    ...
    for i in range(gpu_count):
        ...
        partial_input = f"temp_input_{i}_{config['hier_summ_column_name']}.csv"
        partial_output = f"temp_output_{i}_{config['hier_summ_column_name']}.csv"
    ...
    if not frames:
        print("[generate_summ_data_mgpu] No partial results to combine. Exiting.")
    ...
    print(f"[generate_summ_data_mgpu] All partial summaries combined into {config['output']}.")
    ...
    for (in_f, out_f) in partial_files:
        ...
        if os.path.exists(out_f):
            os.remove(out_f)
    print("[generate_summ_data_mgpu] Temporary files removed.")
