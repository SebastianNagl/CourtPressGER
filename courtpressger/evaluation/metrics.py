    ...
    def compute(reference: str, generated: str,
        ...
        try:
            reference_tokens = nltk.word_tokenize(
                reference.lower(), language='german')
            generated_tokens = nltk.word_tokenize(
                generated.lower(), language='german')
        except LookupError:
            print(
                "NLTK 'punkt' Tokenizer für Deutsch nicht gefunden. Bitte herunterladen:")
...
class BertScoreMetric:
    ...
    def __init__(self, model_type: Optional[str] = None,
        ...
        if self.model_type is None:
            if self.lang == "de":
                ...
                print(
                    f"Warnung: Kein BERTScore-Modelltyp angegeben. Verwende Standard für lang='{self.lang}': {self.model_type}")
            # Füge hier ggf. weitere Standardmodelle für andere Sprachen hinzu
            # elif self.lang == "en":
            #     self.model_type = "roberta-large"
            else:
                raise ValueError(
                    f"Kein Standard-BERTScore-Modell für Sprache '{self.lang}' definiert. Bitte model_type angeben.")
    ...
    def compute(self, reference: str, generated: str) -> Dict[str, float]:
        ...
        try:
            print(
                f"INFO: Berechne BERTScore mit lokalem Modell '{self.model_type}' und num_layers=12 (bert-score v0.3.12)...")
            try:
                ...
                # Dieses Modell ist in bert-score direkt unterstützt
                fallback_model = "bert-base-multilingual-cased"
                print(
                    f"WARNUNG: BERTScore mit {self.model_type} fehlgeschlagen ({e}). Fallback zu {fallback_model}...")
            ...
            print(
                f"Fehler bei BERTScore-Berechnung (Modell: {self.model_type}, Lang: {self.lang}): {e}")
...
class ContentOverlapMetrics:
    ...
    def keyword_overlap(reference: str, generated: str, num_keywords: int = 20) -> float:
        ...
        ref_keywords = set(
            [word for word, _ in ref_counter.most_common(num_keywords)])
        gen_keywords = set(
            [word for word, _ in gen_counter.most_common(num_keywords)])
    ...
    def entity_f1(reference: str, generated: str) -> Dict[str, float]:
        ...
        try:
            ...
            if not ref_entities and not gen_entities:
                return {
                    # Wenn keine Entitäten erwartet wurden und keine gefunden wurden
            ...
            if not ref_entities:
                return {
                    # Wenn keine Entitäten erwartet wurden, aber welche generiert wurden
                    "entity_precision": 0.0,
            ...
            precision = intersection / \
                len(gen_entities) if gen_entities else 0.0
...
class QAGSMetric:
    ...
    def generate_questions(self, text: str) -> List[str]:
        ...
        try:
            ...
            valid_questions = [q for q in questions if len(
                q.split()) >= 3 and ("?" in q)]
        ...
        try:
            ...
            if BERT_SCORE_AVAILABLE:
                ...
                for src_ans, gen_ans in zip(source_answers, generated_answers):
                    ...
                    overlap = len(src_words.intersection(gen_words)
                                  ) / len(src_words.union(gen_words))
...
class FactCCMetric:
    ...
    def compute(self, source: str, generated: str) -> Dict[str, float]:
        ...
        consistent_claims = sum(
            1 for score in consistency_scores if score >= self.threshold)
...
class LLMAsJudgeMetric:
    ...
    def __init__(self,
        ...
        if not self.api_key:
            raise ValueError(
                "ANTHROPIC_API_KEY muss als Parameter oder Umgebungsvariable gesetzt sein")
        ...
        try:
            ...
            raise ImportError(
                "Anthropic-Paket muss installiert sein. Führe 'uv add anthropic' aus.")
...
def compute_all_metrics(reference: str, generated: str,
    ...
    metrics['keyword_overlap'] = ContentOverlapMetrics.keyword_overlap(
        reference, generated)
    metrics['entity_overlap'] = ContentOverlapMetrics.entity_overlap(
        reference, generated)
    metrics['length_ratio'] = ContentOverlapMetrics.length_ratio(
        reference, generated)

    # Entity-F1-Score berechnen (falls spaCy verfügbar)
    if SPACY_AVAILABLE:
        try:
            entity_f1_metrics = ContentOverlapMetrics.entity_f1(
                reference, generated)
    ...
    if BERT_SCORE_AVAILABLE and bert_score_model:
        try:
            print(
                f"INFO: Berechne BERTScore mit lokalem Modell '{bert_score_model}' und num_layers=12 (bert-score v0.3.12)...")
            try:
                # BERTScore mit dem angegebenen Modell berechnen
                P, R, F1 = bert_score.score(
                    [generated], [reference],
                ...
                fallback_model = "bert-base-multilingual-cased"
                print(
                    f"WARNUNG: BERTScore mit {bert_score_model} fehlgeschlagen ({e}). Fallback zu {fallback_model}...")
            ...
            import traceback
            print(
                f"Fehler bei der Berechnung des BERTScores mit Modell {bert_score_model}:", file=sys.stderr)
            traceback.print_exc()
    ...
    if semantic_similarity_model and TRANSFORMERS_AVAILABLE:
        try:
            similarity_metric = SemanticSimilarityMetric(
                model_name=semantic_similarity_model)
            metrics['semantic_similarity'] = similarity_metric.compute(
                reference, generated)
        except Exception as e:
            print(
                f"Fehler bei der Berechnung der semantischen Ähnlichkeit: {str(e)}")
    ...
    if enable_factual_consistency and source_text and TRANSFORMERS_AVAILABLE:
        ...
        try:
            ...
            print(
                f"INFO: QAGS-Score berechnet: {qags_results.get('qags_score', 'N/A')}")
        except Exception as e:
        ...
        try:
            ...
            print(
                f"INFO: FactCC-Score berechnet: {factcc_results.get('factcc_score', 'N/A')}")
        except Exception as e:
    ...
    if enable_llm_as_judge and source_text:
        try:
            ...
            llm_judge_results = llm_judge.compute(
                source_text, generated, reference)
            metrics.update(llm_judge_results)
            print(
                f"INFO: LLM-as-Judge Score berechnet: {llm_judge_results.get('llm_judge_gesamtscore', 'N/A')}")
        except Exception as e:
