...
def sample_dataset():
    data = {
        ...
        # Beispiel mit fehlerhaftem Eintrag
        'model_b': ['Generierte PM 1 von B', 'Generierte PM 2 von B mit Fehler'],
        # Beispiel mit fehlendem Eintrag
        'model_c': [None, 'Generierte PM 2 von C']
    }
...
def evaluation_pipeline(tmp_path):
    ...
    models_to_evaluate = ['model_a', 'model_b', 'model_c',
        'model_d_missing']  # model_d fehlt im Dataset
    pipeline = LLMEvaluationPipeline(
        models_or_names=models_to_evaluate, output_dir=str(output_dir))
...
class TestLLMEvaluationPipeline:
    ...
    def test_init(self, evaluation_pipeline):
        ...
        # Prüfen, ob das Ausgabeverzeichnis erstellt wurde
        assert os.path.exists(output_dir)
        assert isinstance(pipeline.models_or_names, list)
        assert pipeline.models_or_names == [
            'model_a', 'model_b', 'model_c', 'model_d_missing']
    ...
    def test_run_evaluation_existing_columns(self, mock_file_open, mock_compute_all, mock_rouge_scorer,
        ...
        # Modell C wird evaluiert, aber mit Fehlern für den ersten Eintrag
        ...
        write_calls = [call_args[0][0]
            for call_args in mock_file_open.call_args_list if call_args[0][1] == 'w']
        ...
        # Einzelne LLM-as-a-Judge Metriken
        assert 'avg_llm_judge_faktische_korrektheit' in summary_a
        # Model A hat 2 erfolgreiche Generierungen
        ...
        # Nur der zweite Eintrag ist gültig
        assert summary_c['successful_generations'] == 1
        # Der erste Eintrag war None
        assert summary_c['failed_generations'] == 1
    ...
    def test_compute_summary_with_results(self, evaluation_pipeline):
        ...
        # Nicht im Beispiel-Result-Dict enthalten
        assert 'avg_rouge1_precision' not in summary


<< << << < HEAD
== == == =
   assert summary['avg_llm_judge_gesamtscore'] == pytest.approx(
       (8.0 + 7.0) / 2)
    assert summary['avg_llm_judge_faktische_korrektheit'] == pytest.approx(
        (8 + 7) / 2)
    assert summary['avg_llm_judge_vollständigkeit'] == pytest.approx(
        (7 + 6) / 2)
>>>>>> > master
   ...
   def _mock_open_side_effect(self, checkpoint_path, checkpoint_content):
    ...
    def test_run_evaluation_with_checkpoint(self, mock_exists, mock_file_open, mock_compute_all, mock_rouge_scorer,
        ...
        checkpoint_path = os.path.join(
            output_dir, f"{model_name}_checkpoint.json")
...
>> >>>>> master
        ...
        mock_file_open.side_effect = self._mock_open_side_effect(
        ...
        read_calls = [call_args[0][0]
            for call_args in mock_file_open.call_args_list if call_args[0][1] == 'r']
